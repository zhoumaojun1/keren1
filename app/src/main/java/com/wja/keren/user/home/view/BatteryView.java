...
public class BatteryView extends View {
    ...
    public BatteryView(Context context, AttributeSet attrs) {
        ...
        mColor = typedArray.getColor(R.styleable.Battery_batteryColor, R.color.white);
...
//        }
        if (mPower >= 20) {
            if (mContext!=null){
                paint.setColor(mContext.getColor(R.color.color_1FC8A9));
            }
        }
        canvas.drawRect(r2, paint);
        //画电池头
        RectF r3 = new RectF(width - strokeWidth, height * 0.25f, width+102, height * 0.75f);
        //设置电池头颜色为黑色
     ...
     * @param canvas
     */
    private void drawivAddCard(Canvas canvas) {
        Paint paint = new Paint();
        paint.setColor(mColor);
        paint.setStyle(Paint.Style.STROKE);
     ...
     * @param power
     */
    public void setPower(Context context,int power) {
        mContext = context;
        this.mPower = power;
        if (mPower < 0) {
            mPower = 100;
